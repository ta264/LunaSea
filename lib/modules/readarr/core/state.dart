...
class ReadarrState extends LunaModuleState {
  ...
  ReadarrSignalR? get signalr => _signalr;
  ...
  void resetProfile() {
    ...
    if (_signalr != null) {
      _signalr!.stop().then((_) => _signalr = null);
    }
  ...
  void onMessage(SignalRMessage message) {
    switch (message.name) {
      case 'author':
        return handleAuthor(message.body!);
      case 'book':
        return handleBook(message.body!);
      case 'tag':
        return handleTag(message.body!);
      case 'Rootfolder':
        return handleRootFolder(message.body!);
    }
  }

  void handleAuthor(Map<String, dynamic> json) {
    final message = ResourceChangeMessage<ReadarrAuthor>.fromJson(
        json, (json) => ReadarrAuthor.fromJson(json as Map<String, dynamic>));

    if (message.action == ModelAction.UPDATED) {
      setSingleAuthor(message.resource!);
    } else if (message.action == ModelAction.DELETED) {
      removeSingleAuthor(message.resource!.id!);
    }
  }

  void handleBook(Map<String, dynamic> json) {
    final message = ResourceChangeMessage<ReadarrBook>.fromJson(
        json, (json) => ReadarrBook.fromJson(json as Map<String, dynamic>));

    if (message.action == ModelAction.UPDATED) {
      setSingleBook(message.resource!);
    } else if (message.action == ModelAction.DELETED) {
      removeSingleBook(message.resource!.id!);
    }
  }

  void handleTag(Map<String, dynamic> json) {
    final message = ResourceChangeMessage<ReadarrTag>.fromJson(
        json, (json) => ReadarrTag.fromJson(json as Map<String, dynamic>));
  ...
  void handleRootFolder(Map<String, dynamic> json) {
    final message = ResourceChangeMessage<ReadarrRootFolder>.fromJson(json,
        (json) => ReadarrRootFolder.fromJson(json as Map<String, dynamic>));
